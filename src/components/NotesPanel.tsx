...
const initialMockNotes: Note[] = [
  { id: 1, content: "函数的三要素：定义域、值域、对应法则。", associatedNodeId: "1-1" },
...
export function NotesPanel({ className = "" }: IclassName) {
  ...
  const handleAddNote = () => {
    ...
    const newNote: Note = {
      ...
      associatedNodeId: selectedNode?.id ?? null,
    };
    ...
    <aside className={cn("w-80 flex flex-col p-4 bg-white", className)}>
      <div className="flex-shrink-0 pb-2 border-b-2 border-gray-200">
        <h2 className="text-lg font-bold text-gray-800">
          {selectedNode ? `关于「${selectedNode.label}」的笔记` : "全局笔记"}
        </h2>
      </div>
      ...
      <div className="flex-1 my-4 space-y-3 overflow-y-auto">
        {filteredNotes.length > 0 ? (
          filteredNotes.map(note => (
            <div key={note.id} className="p-3 bg-yellow-50 rounded-lg group relative animate-in fade-in duration-300">
          ...
          <div className="text-center text-gray-400 pt-10">
            {/* 【修改】优化了提示文本 */}
            {selectedNode ? "暂无相关笔记，快写下第一条吧！" : "暂无全局笔记，快写下第一条吧！"}
          </div>
        )}
      </div>
